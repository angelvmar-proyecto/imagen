...
import { applyRuntimeThreads, applySingleThreadRuntime, applyWebRuntimeConfig, } from "./runtime-config";
import { fromOrtTensor, toOrtTensor } from "./tensor";
export class CapacitorOnnxWeb {
    constructor() {
        this.sessions = new Map();
    }
    static modelKey(modelId, version) {
        return `model-${modelId}-${version}`;
    }
    async loadModel(_input) {
        if (_input.filePath !== undefined) {
            throw new CapacitorOnnxError("MODEL_INVALID", "filePath is not supported on web; pass modelBuffer (Uint8Array) instead");
        }
        if (!_input.modelBuffer) {
            throw new CapacitorOnnxError("MODEL_INVALID", "Expected exactly one of filePath or modelBuffer");
        }
    ...
    async run(_input) {
        ...
        if (!session) {
            throw new CapacitorOnnxError("SESSION_INIT_ERROR", "ONNX Runtime session is not initialized. Please load a model before running inference.");
        }
        const inputNames = Object.keys(_input.inputs ?? {});
        if (inputNames.length === 0) {
            throw new CapacitorOnnxError("INFERENCE_ERROR", "run() requires at least one input tensor in `inputs`.");
        }
        const feeds = {};
        for (const name of inputNames) {
            if (!session.inputNames.includes(name)) {
                throw new CapacitorOnnxError("INFERENCE_ERROR", `Model has no input named '${name}'.`);
            }
    ...
    static async warmupSession(session, warmupInputs) {
        ...
        try {
            ...
            console.warn("[CapacitorOnnxWeb] Warmup inference failed; continuing without warmup.", err);
            return false;
        }
    }
}
