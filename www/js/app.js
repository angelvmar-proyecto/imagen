async function procesarArchivoDual(excelFile, imageFile) {
    let datosExcel = [];
    if (excelFile) { datosExcel = await leerEstructuraExcel(excelFile); }
    ...
    console.log('Iniciando OCR con Tesseract local...');
    const resultadosOCR = await extraerTextoRealConTesseract(ctx, canvas, lineasVisualesY, datosExcel);
    if (excelFile && datosExcel.length > 0) { calibrarYRegistrarAprendizaje(datosExcel, lineasVisualesY); }
    if (typeof mostrarLogEnPantalla === 'function') { mostrarLogEnPantalla(); }
...
function detectarGeometriaEstructuralPura(imgData, width, height) {
    ...
    for (let y = 0; y < height; y++) {
        ...
        for (let x = 0; x < width; x += 3) {
            ...
            if (Math.abs(luma - lumaAnterior) > 20) { cambiosBorde++; }
            lumaAnterior = luma;
        }
    ...
    let ultimoY = -30;
    cortesY.forEach(y => {
        if (y - ultimoY > 20) { filasEstructurales.push(y); ultimoY = y; }
    });
...
async function extraerTextoRealConTesseract(ctx, canvas, lineasY, datosExcel) {
    ...
    const worker = await Tesseract.createWorker('spa+eng', 1, { langPath: './tessdata' });
    for (let i = 0; i < totalFilas; i++) {
        ...
        let textoExtraido = 'Fila ' + (i+1) + ' (Sin texto)';
        try {
            const dataURL = subCanvas.toDataURL('image/png');
            const ret = await worker.recognize(dataURL);
            if (ret && ret.data && ret.data.text) { textoExtraido = ret.data.text.trim().replace(/
/g, ' | '); }
        } catch (err) { console.error('Error OCR:', err); }
        let referenciaExcel = (datosExcel[i] && datosExcel[i].join) ? datosExcel[i].join(' | ') : 'Registro ' + (i+1);
        filasExtraidas.push({ id: i + 1, columnas: [textoExtraido !== '' ? textoExtraido : 'Vacío'], referencia: referenciaExcel, cordYAsignada: yInicio, calibracionEstado: 'OCR Real Local' });
    }
...
function calibrarYRegistrarAprendizaje(datosExcel, lineasVisualesY) {
    ...
    let patron = { timestamp: new Date().toISOString(), filasExcelCount: datosExcel.length, lineasVisualesCount: lineasVisualesY.length, mapaMuestras: lineasVisualesY.slice(0, datosExcel.length) };
...
async function ejecutarCalibracionDual() {
    try {
        ...
        document.getElementById('tabla-resultados').querySelector('tbody').innerHTML = '<tr><td colspan="3" style="text-align:center; color:#ffeb3b;">Procesando con Tesseract local...</td></tr>';
    ...
    } catch (error) { console.error(error); alert('Error OCR: ' + error.message); }
}
